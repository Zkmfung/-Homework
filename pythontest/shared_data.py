...
def delete_city(del_city):      # 删除城市
    if del_city not in cities:  # 城市不存在，跳过
        ...
        if del_city in train_time.keys():
            del train_time[del_city]  # 时刻表中删除
        for city in train_time:
            if del_city in train_time[city].keys():
                train_time[city].pop(del_city)
        if del_city in train_cost.keys():
            del train_cost[del_city]  # 价格表中删除
        for city in train_cost:
            if del_city in train_cost[city].keys():
                train_cost[city].pop(del_city)
